 ...
 * Libdmrconf is a QT based library providing methods to assemble, read and programm codeplugs for
 ...
 *
 * The aim of this project, is to define a minimal but sufficient common configuration for all
 * radios, such that all relevant ham-radio features of the radios can be used. This is a challange
 * as there are many different manufacturers, each selling serveral different models, each having
 * a varity of different features. In fact, each manufacturer provides a separate application for
 * each model to programm the radios. The resulting codeplugs are usually stored in a binary format
 * similar to the actual codeplug bing loaded onto the radio.
 *
 * This makes absolutely sense for commercial applications, like radios used by staff at large
 * events. In this case, the organizer of the event would buy large quantities of identical radios,
 ...
 * are specific to the commercial application and are not used in ham-radio.
 *
 ...
 * documented in the @link conffile config-file module.
 *
 */
