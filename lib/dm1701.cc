...
DM1701::DM1701(QObject *parent)
  : Radio(parent), _name("Baofeng DM-1701"), _dev(nullptr), _codeplugFlags(), _config(nullptr)
{
  // pass...
}
...
bool
DM1701::startDownload(bool blocking) {
  if (StatusIdle != _task)
    return false;
...
bool
DM1701::startUpload(Config *config, bool blocking, const CodePlug::Flags &flags) {
  if (StatusIdle != _task)
    return false;
...
bool
DM1701::startUploadCallsignDB(UserDatabase *db, bool blocking, const CallsignDB::Selection &selection) {
  if (StatusIdle != _task)
    return false;
...
DM1701::uploadCallsigns() {
  ...
  _dev->erase(_callsigns.image(0).element(0).address(),
              ...
              [](uint percent, void *ctx) { emit ((DM1701 *)ctx)->uploadProgress(percent/2); }, this);
