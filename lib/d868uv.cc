...
D868UV::D868UV(QObject *parent)
  : Radio(parent), _name("Anytone AT-D868UV"), _dev(nullptr), _codeplugFlags(), _config(nullptr)
{
  // pass...
}
...
bool
D868UV::startDownload(bool blocking) {
  if (StatusIdle != _task)
    return false;
...
bool
D868UV::startUpload(Config *config, bool blocking, const CodePlug::Flags &flags) {
  if (StatusIdle != _task)
    return false;
...
bool
D868UV::startUploadCallsignDB(UserDatabase *db, bool blocking, const CallsignDB::Selection &selection) {
  Q_UNUSED(db);
  Q_UNUSED(blocking);
