...
DFUFile::dump(QTextStream &stream) const {
  stream << "DFU file with " << _images.size() << " images:" << Qt::endl;
  foreach (const Image &i, _images) {
    i.dump(stream);
  }
}
...
DFUFile::Element::dump(QTextStream &stream) const {
  stream << "  Element @ 0x" << Qt::hex << _address << ", size=0x" << Qt::hex << _data.size() << Qt::endl;
  ...
  for (int i=0; i<nrow; i++) {
    ...
    if ((i>0) && (0==memcmp(last_line, _data.constData()+i*16, 16))) {
      skipping = true;
      stream << qSetFieldWidth(8) << Qt::right << "*" << qSetFieldWidth(1) << Qt::endl;
      continue;
    }
    memcpy(last_line, _data.constData()+i*16, 16);
    skipping = false;
    stream << qSetFieldWidth(8) << Qt::right << Qt::hex << (_address+i*16)
           << qSetFieldWidth(1) << "  ";
    for (int j=(i*16); j<(i*16+8); j++) {
      stream << qSetFieldWidth(2) << Qt::right << Qt::hex << uint8_t(_data.at(j))
             << qSetFieldWidth(1) << " ";
    }
    stream << " ";
...
DFUFile::Image::dump(QTextStream &stream) const {
  ...
  if (_name.isEmpty())
    ...
    stream << ", target '" << _name << "'";
  stream << ", #elements=" << _elements.size() << ":" << Qt::endl;
  foreach (const Element &e, _elements) {
    e.dump(stream);
  }
}
